...
const overlap = (elfs: IElf[]): boolean => {
  if (elfs[0].min >= elfs[1].min || elfs[0].max <= elfs[1].max) return true;
  if (elfs[1].min >= elfs[0].min || elfs[1].max <= elfs[0].max) return true;
  return false;
};

const input = fs
  .readFileSync("./day_4/input_prod.txt", "utf8")
  .split("\n")
